...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a DWE address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no DWE: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert dwe:// to dwe:
    //
    //    Cannot handle this later, because dwe:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open dwe.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DWE.lnk";
}
...
{
    return GetAutostartDir() / "dwe.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a dwe.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the dwe app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add dwe app to startup item list
