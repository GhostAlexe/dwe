...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDweAmount"))
        settings.setValue("nAnonymizeDweAmount", 1000);

    nAnonymizeDweAmount = settings.value("nAnonymizeDweAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDweAmount"))
        SoftSetArg("-anonymizedweamount", settings.value("nAnonymizeDweAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in dwe.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDweAmount:
            return QVariant(nAnonymizeDweAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDweAmount:
            nAnonymizeDweAmount = value.toInt();
            settings.setValue("nAnonymizeDweAmount", nAnonymizeDweAmount);
            emit anonymizeDweAmountChanged(nAnonymizeDweAmount);
            break;
        case CoinControlFeatures:
